...
@Service
public class PK8Exporter extends TableExcelExporter<PK8Data>{
    ...
    protected void createCells(XSSFRow row, PK8Data entity, XSSFCellStyle style) {
    ...
    public void exportToStream(ByteArrayOutputStream stream, String year) throws IOException {
        ExcelFileId excelFileId = new ExcelFileId("PK8", "2021");
        ...
        writeDataLines(workbook.getSheetAt(0), initObjects(pk_8_Repository.getQueryResult()));
        writeDataLines(workbook.getSheetAt(1), initObjects(pk_8_Repository.selectByKTK()));
        writeDataLines(workbook.getSheetAt(2), initObjects(pk_8_Repository.selectBySTK()));

        completeBorders(workbook);

        workbook.write(stream);
        excelFileRepository.save(new ExcelFile("PK8", "2021", stream.toByteArray()));
        workbook.close();
    }

    private List<AdministrationPK8_2> initObjects(List<PK8Data> entities) {
